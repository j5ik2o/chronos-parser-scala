...
object CronParser {
  ...
  private[cron] def hourDigit_00_09[$: P]: P[ValueExpr] =
    P("0" ~ CharIn("0-9") | CharIn("0-9")).!.map(s => ValueExpr(s.toInt))
  private[cron] def hourDigit_10_19[$: P]: P[ValueExpr] = P("1" ~ CharIn("0-9")).!.map(s => ValueExpr(s.toInt))
  private[cron] def hourDigit_20_23[$: P]: P[ValueExpr] = P("2" ~ CharIn("0-3")).!.map(s => ValueExpr(s.toInt))
  private[cron] def hourDigit[$: P]: P[ValueExpr]       = P(hourDigit_20_23 | hourDigit_10_19 | hourDigit_00_09)
  ...
  private[cron] def dayDigit_30_31[$: P]: P[ValueExpr] = P("3" ~ CharIn("0-1")).!.map(s => ValueExpr(s.toInt))
  private[cron] def dayDigit[$: P]: P[ValueExpr]       = P(dayDigit_30_31 | dayDigit_10_29 | dayDigit_01_09)

  private[cron] def monthDigit_00_09[$: P]: P[ValueExpr] =
    P("0" ~ CharIn("1-9") | CharIn("1-9")).!.map(s => ValueExpr(s.toInt))
  private[cron] def monthDigit_10_12[$: P]: P[ValueExpr] = P("1" ~ CharIn("0-2")).!.map(s => ValueExpr(s.toInt))
  private[cron] def monthDigit[$: P]: P[ValueExpr]       = P(monthDigit_10_12 | monthDigit_00_09)

  private[cron] def dayOfWeekDigit[$: P]: P[ValueExpr] = P(CharIn("1-7")).!.map(s => ValueExpr(s.toInt))
  ...
  private[cron] def asteriskPer[$: P](digit: => P[Expr]): P[PerExpr] = P(asterisk ~ per(digit)).map { case (d, op) =>
    PerExpr(d, op)
  }
  ...
  private[cron] def range[$: P](digit: => P[Expr]): P[RangeExpr] = P(digit ~ "-" ~ digit ~ rangePer(digit)).map {
    case (from, to, per) =>
      RangeExpr(from, to, per)
  }

  private[cron] def list[$: P](digit: => P[Expr]): P[Expr] = P(digit.rep(sep = ",")).map(_.toList).map {
    case x :: Nil => x
    case l        => ListExpr(l)
  }
